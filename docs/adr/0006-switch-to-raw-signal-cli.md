...
*   **Startup-race avoidance:** Because of `--receive-mode on-connection`, the `signal-cli` daemon will *not* pull messages from the Signal servers until our Node.js worker actively connects to the TCP socket. This removes the startup race under normal operation; the bounded receive and shutdown boundaries are documented in ADR 0019.
...
*   **No self-echo loop:** Outgoing responses use `notifySelf: true` so they are delivered to linked devices as incoming self-chat messages. Self-chat `syncMessage.sentMessage` events are accepted only when both source and destination are the configured account and the source device is in the explicit allowlist of user device IDs; all other self-chat devices are treated as outgoing self-echoes.
